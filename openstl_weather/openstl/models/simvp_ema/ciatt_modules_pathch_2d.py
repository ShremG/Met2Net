...
class PatchEmbedding(nn.Module):
    def __init__(self, d_model: int = 256, patch_size: int = 4, img_size= (16, 32)):
        super().__init__()
        self.patch_size = patch_size
        num_patchs = (img_size[0] // patch_size)*(img_size[1] // patch_size)
        self.positions = nn.Parameter(torch.randn(num_patchs, d_model,patch_size,patch_size))
                
    def forward(self, x):
        b,c,h,w = x.shape
        pathes = rearrange(x, 'b c (h s1) (w s2) -> b c (h w) s1 s2', s1=self.patch_size, s2=self.patch_size)
        pathes = pathes.reshape(b,c,-1,self.patch_size,self.patch_size).permute(0,2,1,3,4)
        pathes += self.positions
        return pathes



class CIAttBlock(nn.Module):
    def __init__(self, d_model, heads, patch_size: int = 4, img_size= (16, 32)):
        super().__init__()
        self.d_model = d_model
        self.patch_emb = PatchEmbedding(d_model=d_model,patch_size=patch_size,img_size=img_size)
        self.patch_size = patch_size
        self.numx = img_size[0]//patch_size
        self.numy = img_size[1]//patch_size
    ...
    def forward(self, x):
        b, s, c, h, w = x.size()
        x = self.patch_emb(x.reshape(b*s,c,h,w)).reshape(b,-1,c,self.patch_size,self.patch_size)

        x = x + self.attn_1(x, x, x) # q k v
        x = rearrange(x, 'b (s numx numy) c p1 p2 -> b s c (p1 numx) (p2 numy)', p1=self.patch_size, p2=self.patch_size,numx=self.numx,numy=self.numy)
...
class CIMidNet(nn.Module):
    def __init__(self, in_channels, d_model, n_layers, heads):
        ...
        self.layers = get_clones(CIAttBlock(d_model, heads, patch_size=4,img_size=(16,32)), n_layers)
