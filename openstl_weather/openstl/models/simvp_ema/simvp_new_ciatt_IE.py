...
class SimVP_Model(nn.Module):
    ...
    def forward(self, x_raw, y_raw, **kwargs):
        ...
        y = y.view(B*T1, C, H, W)
        # 一阶段
        h_u10, _ = self.enc_u10_q(x[:,0:3,:,:].reshape(B*T*3,1,H,W))
        h_v10, _ = self.enc_v10_q(x[:,3:6,:,:].reshape(B*T*3,1,H,W))
        h_t2m, _ = self.enc_t2m_q(x[:,6:9,:,:].reshape(B*T*3,1,H,W))
        h_tcc, _ = self.enc_tcc_q(x[:,9:12,:,:].reshape(B*T*3,1,H,W))
        H_, W_ = h_tcc.shape[-2],h_tcc.shape[-1]

        z_x = torch.stack((h_u10, h_v10, h_t2m, h_tcc), dim=1) # b*t*3,4,c,h,w
        z_x = z_x.reshape(B,T,12,-1,H_, W_).permute(0,2,1,3,4,5).reshape(B,12,-1,H_, W_)
        self.hid_k.train()
        z_y_pre = self.hid_k(z_x)
        z_y_pre = z_y_pre.reshape(B,12,T,-1,H_, W_).permute(0,2,1,3,4,5).reshape(B*T*3,4,-1,H_, W_)
        h_pre_u10 = z_y_pre[:,0,:,:]
        h_pre_v10 = z_y_pre[:,1,:,:]
        h_pre_t2m = z_y_pre[:,2,:,:]
        h_pre_tcc = z_y_pre[:,3,:,:]
        ...
        h_u10, _ = self.enc_u10_k(x[:,0:3,:,:].reshape(B*T*3,1,H,W))
        h_v10, _ = self.enc_v10_k(x[:,3:6,:,:].reshape(B*T*3,1,H,W))
        h_t2m, _ = self.enc_t2m_k(x[:,6:9,:,:].reshape(B*T*3,1,H,W))
        h_tcc, _ = self.enc_tcc_k(x[:,9:12,:,:].reshape(B*T*3,1,H,W))
        H_, W_ = h_tcc.shape[-2],h_tcc.shape[-1]
        z_x = torch.stack((h_u10, h_v10, h_t2m, h_tcc), dim=1) # b*t*3,4,c,h,w
        z_x = z_x.reshape(B,T,12,-1,H_, W_).permute(0,2,1,3,4,5).reshape(B,12,-1,H_, W_)
        self.hid_k.train()
        ...
        h_y_u10, _ = self.enc_u10_k(y[:,0:3,:,:].reshape(B*T*3,1,H,W))
        h_y_v10, _ = self.enc_v10_k(y[:,3:6,:,:].reshape(B*T*3,1,H,W))
        h_y_t2m, _ = self.enc_t2m_k(y[:,6:9,:,:].reshape(B*T*3,1,H,W))
        h_y_tcc, _ = self.enc_tcc_k(y[:,9:12,:,:].reshape(B*T*3,1,H,W))
        z_y = torch.stack((h_y_u10, h_y_v10, h_y_t2m, h_y_tcc), dim=1) # b*t*3,4,c,h,w
