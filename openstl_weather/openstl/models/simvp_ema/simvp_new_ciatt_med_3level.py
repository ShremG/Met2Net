...
class SimVP_Model(nn.Module):
    ...
    def __init__(self, in_shape, hid_S=16, hid_T=256, N_S=4, N_T=4, model_type='gSTA',
        ...
        self.dec_k_s = []
        for i in range(C):
            self.enc_q = Encoder(1, hid_S, N_S, spatio_kernel_enc, act_inplace=act_inplace)
            self.enc_k = Encoder(1, hid_S, N_S, spatio_kernel_enc, act_inplace=act_inplace)
            self.enc_q_s.append(self.enc_q)
            self.enc_k_s.append(self.enc_k)

            self.dec_q = Decoder(hid_S, 1, N_S, spatio_kernel_dec, act_inplace=act_inplace)
            self.dec_k = Decoder(hid_S, 1, N_S, spatio_kernel_dec, act_inplace=act_inplace)
            self.dec_q_s.append(self.dec_q)
            self.dec_k_s.append(self.dec_k)

        self.enc_q_s = nn.ModuleList(self.enc_q_s)
        self.enc_k_s = nn.ModuleList(self.enc_k_s)
        self.dec_q_s = nn.ModuleList(self.dec_q_s)
        self.dec_k_s = nn.ModuleList(self.dec_k_s)
    ...
    def init_weights(self):
        ...
        for i in range(self.C):
            for param_q, param_k in zip(self.enc_q_s[i].parameters(),
                                        self.enc_k_s[i].parameters()):
                param_k.requires_grad = False
                param_k.data.copy_(param_q.data)
        
            for param_q, param_k in zip(self.dec_q_s[i].parameters(),
                                        self.dec_k_s[i].parameters()):
                param_k.requires_grad = False
                param_k.data.copy_(param_q.data)
    ...
    def forward(self, x_raw, y_raw, **kwargs):
        ...
        h_s = []
        for i in range(self.C):
            h_i,_ = self.enc_q_s[i](x[:,i:i+1,:,:])
            h_s.append(h_i)
        
        H_, W_ = h_s[-1].shape[-2],h_s[-1].shape[-1]

        z_x = torch.stack(h_s, dim=1) # b*t,12,c,h,w
        z_x = z_x.reshape(B,T,self.C,-1,H_, W_).permute(0,2,1,3,4,5).reshape(B,self.C,-1,H_, W_)
        self.hid_k.train()
        z_y_pre = self.hid_k(z_x)
        z_y_pre = z_y_pre.reshape(B,self.C,T,-1,H_, W_).permute(0,2,1,3,4,5).reshape(B*T,self.C,-1,H_, W_)
        rec_s = []
        for i in range(self.C):
            rec_i = self.dec_q_s[i](z_y_pre[:,i,...])
            rec_s .append(rec_i)

        rec_y = torch.stack(rec_s, dim=1)
        ...
        h_s = []
        for i in range(self.C):
            h_i,_ = self.enc_k_s[i](x[:,i:i+1,:,:])
            h_s.append(h_i)

        z_x = torch.stack(h_s, dim=1) # b*t,self.c,c,h,w
        z_x = z_x.reshape(B,T,self.C,-1,H_, W_).permute(0,2,1,3,4,5).reshape(B,self.C,-1,H_, W_)
        ...
        h_y_s = []
        for i in range(self.C):
            h_i,_ = self.enc_k_s[i](y[:,i:i+1,:,:])
            h_y_s.append(h_i)
    
        z_y = torch.stack(h_y_s, dim=1) # b*t,4,c,h,w
        loss_latent = F.mse_loss(z_y_pre,z_y)

        pre_s = []
        for i in range(self.C):
            pre_i = self.dec_k_s[i](z_y_pre[:,i,...])
            pre_s.append(pre_i)

        pre_y = torch.stack(pre_s, dim=1)
