...
class SimVP_Model(nn.Module):
    ...
    def forward(self, x_raw, y_raw, **kwargs):
        ...
        self.hid_k.train()
        z_y_pre = self.hid_k(z_x).reshape(B*T,-1,H_,W_)
        h_pre_u10 = z_y_pre[:,:self.hid_s,:,:]
        h_pre_v10 = z_y_pre[:,self.hid_s:2*self.hid_s,:,:]
        h_pre_t2m = z_y_pre[:,2*self.hid_s:3*self.hid_s,:,:]
        h_pre_tcc = z_y_pre[:,3*self.hid_s:4*self.hid_s,:,:]
