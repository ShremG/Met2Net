...
class SimVP_Model(nn.Module):
    ...
    def __init__(self, in_shape, hid_S=16, hid_T=256, N_S=4, N_T=4, model_type='gSTA',
                 ...
                 spatio_kernel_dec=3, act_inplace=True, momentum_ema=0.9,dim_ae=8,**kwargs):
        ...
        self.enc_u10_q = Encoder(1, hid_S, N_S, spatio_kernel_enc, act_inplace=act_inplace, dim_ae=dim_ae)
        self.enc_u10_k = Encoder(1, hid_S, N_S, spatio_kernel_enc, act_inplace=act_inplace, dim_ae=dim_ae)

        self.enc_v10_q = Encoder(1, hid_S, N_S, spatio_kernel_enc, act_inplace=act_inplace, dim_ae=dim_ae)
        self.enc_v10_k = Encoder(1, hid_S, N_S, spatio_kernel_enc, act_inplace=act_inplace, dim_ae=dim_ae)

        self.enc_t2m_q = Encoder(1, hid_S, N_S, spatio_kernel_enc, act_inplace=act_inplace, dim_ae=dim_ae)
        self.enc_t2m_k = Encoder(1, hid_S, N_S, spatio_kernel_enc, act_inplace=act_inplace, dim_ae=dim_ae)

        self.enc_tcc_q = Encoder(1, hid_S, N_S, spatio_kernel_enc, act_inplace=act_inplace, dim_ae=dim_ae)
        self.enc_tcc_k = Encoder(1, hid_S, N_S, spatio_kernel_enc, act_inplace=act_inplace, dim_ae=dim_ae)

        self.dec_u10_q = Decoder(hid_S, 1, N_S, spatio_kernel_dec, act_inplace=act_inplace, dim_ae=dim_ae)
        self.dec_u10_k = Decoder(hid_S, 1, N_S, spatio_kernel_dec, act_inplace=act_inplace, dim_ae=dim_ae)

        self.dec_v10_q = Decoder(hid_S, 1, N_S, spatio_kernel_dec, act_inplace=act_inplace, dim_ae=dim_ae)
        self.dec_v10_k = Decoder(hid_S, 1, N_S, spatio_kernel_dec, act_inplace=act_inplace, dim_ae=dim_ae)

        self.dec_t2m_q = Decoder(hid_S, 1, N_S, spatio_kernel_dec, act_inplace=act_inplace, dim_ae=dim_ae)
        self.dec_t2m_k = Decoder(hid_S, 1, N_S, spatio_kernel_dec, act_inplace=act_inplace, dim_ae=dim_ae)

        self.dec_tcc_q = Decoder(hid_S, 1, N_S, spatio_kernel_dec, act_inplace=act_inplace, dim_ae=dim_ae)
        self.dec_tcc_k = Decoder(hid_S, 1, N_S, spatio_kernel_dec, act_inplace=act_inplace, dim_ae=dim_ae)

        self.init_weights()

        model_type = 'gsta' if model_type is None else model_type.lower()
        if model_type == 'incepu':
            self.hid = MidIncepNet(T*dim_ae*C, hid_T, N_T)
        else:
            self.hid = MidMetaNet(T*dim_ae*C, hid_T, N_T,
    ...
    def forward(self, x_raw, y_raw, **kwargs):
        ...
        h_pre_u10 = hid[:,:self.dim_ae,:,:]
        h_pre_v10 = hid[:,self.dim_ae:2*self.dim_ae,:,:]
        h_pre_t2m = hid[:,2*self.dim_ae:3*self.dim_ae,:,:]
        h_pre_tcc = hid[:,3*self.dim_ae:4*self.dim_ae,:,:]
        ...
        loss = loss_rec + loss_latent + loss_pre
...
class Encoder(nn.Module):
    ...
    def __init__(self, C_in, C_hid, N_S, spatio_kernel, act_inplace=True,dim_ae=4):
...
class Decoder(nn.Module):
    ...
    def __init__(self, C_hid, C_out, N_S, spatio_kernel, act_inplace=True,dim_ae=4):
...
if __name__ == '__main__':
    ...
    model = SimVP_Model(in_shape=[12,4,32,64],hid_S=32,N_S=2,dim_ae=1).to(device)
