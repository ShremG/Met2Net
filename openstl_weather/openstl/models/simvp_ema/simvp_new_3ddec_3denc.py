...
class SimVP_Model(nn.Module):
    ...
    def forward(self, x_raw, y_raw, **kwargs):
        ...
        y = y.permute(0,2,1,3,4) # b c t h w
        # 一阶段
        ...
        z_x = z_x.permute(0,2,1,3,4) # b t c h w
        self.hid_k.train()
        z_y_pre = self.hid_k(z_x).permute(0,2,1,3,4) # b c t h w
        ...
        # y_blur = self.gs_blur(y.reshape(B*C,T,H,W)).reshape(B,C,T,H,W)
        ...
        # z_y = torch.cat((h_y_u10, h_y_v10, h_y_t2m, h_y_tcc), dim=1).permute(0,2,1,3,4) # b t c h w
        ...
        z_x = torch.cat((h_u10, h_v10, h_t2m, h_tcc), dim=1)
        z_x = z_x.permute(0,2,1,3,4) # b t c h w
        z_y_pre = self.hid_q(z_x).permute(0,2,1,3,4) # b c t h w
        ...
        z_y = torch.cat((h_y_u10, h_y_v10, h_y_t2m, h_y_tcc), dim=1)
...
class Encoder(nn.Module):
    ...
    def __init__(self, C_in, C_hid, N_S, spatio_kernel, act_inplace=True):
        ...
        self.enc = nn.Sequential(
              ConvSC3D(C_in, C_hid, spatio_kernel, downsampling=samplings[0],
                     act_inplace=act_inplace),
            *[ConvSC3D(C_hid, C_hid, spatio_kernel, downsampling=s,
...
class BasicConv3d(nn.Module):
    ...
    def __init__(self,
        ...
        if upsampling is True:
            ...
            self.conv = SamePadConv3d(in_channels, out_channels, kernel_size=4, stride=(1,stride,stride))
            # self.conv = nn.Conv2d(
...
if __name__ == '__main__':
    ...
    model = SimVP_Model(in_shape=[12,4,32,64],hid_S=32,N_S=2).to(device)
