...
class SimVP_Model(nn.Module):
    ...
    def __init__(self, in_shape, hid_S=16, hid_T=256, N_S=4, N_T=4, model_type='gSTA',
        ...
        self.enc_u10_q = Encoder(3, hid_S, N_S, spatio_kernel_enc, act_inplace=act_inplace)
        self.enc_u10_k = Encoder(3, hid_S, N_S, spatio_kernel_enc, act_inplace=act_inplace)

        self.enc_v10_q = Encoder(3, hid_S, N_S, spatio_kernel_enc, act_inplace=act_inplace)
        self.enc_v10_k = Encoder(3, hid_S, N_S, spatio_kernel_enc, act_inplace=act_inplace)

        self.enc_t2m_q = Encoder(3, hid_S, N_S, spatio_kernel_enc, act_inplace=act_inplace)
        self.enc_t2m_k = Encoder(3, hid_S, N_S, spatio_kernel_enc, act_inplace=act_inplace)

        self.enc_tcc_q = Encoder(3, hid_S, N_S, spatio_kernel_enc, act_inplace=act_inplace)
        self.enc_tcc_k = Encoder(3, hid_S, N_S, spatio_kernel_enc, act_inplace=act_inplace)

        self.dec_u10_q = Decoder(hid_S, 3, N_S, spatio_kernel_dec, act_inplace=act_inplace)
        self.dec_u10_k = Decoder(hid_S, 3, N_S, spatio_kernel_dec, act_inplace=act_inplace)

        self.dec_v10_q = Decoder(hid_S, 3, N_S, spatio_kernel_dec, act_inplace=act_inplace)
        self.dec_v10_k = Decoder(hid_S, 3, N_S, spatio_kernel_dec, act_inplace=act_inplace)

        self.dec_t2m_q = Decoder(hid_S, 3, N_S, spatio_kernel_dec, act_inplace=act_inplace)
        self.dec_t2m_k = Decoder(hid_S, 3, N_S, spatio_kernel_dec, act_inplace=act_inplace)

        self.dec_tcc_q = Decoder(hid_S, 3, N_S, spatio_kernel_dec, act_inplace=act_inplace)
        self.dec_tcc_k = Decoder(hid_S, 3, N_S, spatio_kernel_dec, act_inplace=act_inplace)
    ...
    def forward(self, x_raw, y_raw, **kwargs):
        ...
        y = y.view(B*T1, C, H, W)
        # 一阶段
        h_u10, _ = self.enc_u10_q(x[:,0:3,:,:])
        h_v10, _ = self.enc_v10_q(x[:,3:6,:,:])
        h_t2m, _ = self.enc_t2m_q(x[:,6:9,:,:])
        h_tcc, _ = self.enc_tcc_q(x[:,9:12,:,:])
        H_, W_ = h_tcc.shape[-2],h_tcc.shape[-1]

        z_x = torch.stack((h_u10, h_v10, h_t2m, h_tcc), dim=1) # b*t,4,c,h,w
        z_x = z_x.reshape(B,T,4,-1,H_, W_).permute(0,2,1,3,4,5).reshape(B,4,-1,H_, W_)
        self.hid_k.train()
        z_y_pre = self.hid_k(z_x)
        z_y_pre = z_y_pre.reshape(B,4,T,-1,H_, W_).permute(0,2,1,3,4,5).reshape(B*T,4,-1,H_, W_)
        h_pre_u10 = z_y_pre[:,0,:,:]
        h_pre_v10 = z_y_pre[:,1,:,:]
        h_pre_t2m = z_y_pre[:,2,:,:]
        h_pre_tcc = z_y_pre[:,3,:,:]
        ...
        lmd = 1
        h_rec_u10 =  lmd * h_pre_u10
        h_rec_v10 =  lmd * h_pre_v10
        h_rec_t2m =  lmd * h_pre_t2m
        h_rec_tcc =  lmd * h_pre_tcc

        # 3D DEC
        rec_u10 = self.dec_u10_q(h_rec_u10.reshape(B,T,-1,H_,W_).permute(0,2,1,3,4))
        rec_v10 = self.dec_v10_q(h_rec_v10.reshape(B,T,-1,H_,W_).permute(0,2,1,3,4))
        rec_t2m = self.dec_t2m_q(h_rec_t2m.reshape(B,T,-1,H_,W_).permute(0,2,1,3,4))
        rec_tcc = self.dec_tcc_q(h_rec_tcc.reshape(B,T,-1,H_,W_).permute(0,2,1,3,4))
        ...
        h_u10, _ = self.enc_u10_k(x[:,0:3,:,:])
        h_v10, _ = self.enc_v10_k(x[:,3:6,:,:])
        h_t2m, _ = self.enc_t2m_k(x[:,6:9,:,:])
        h_tcc, _ = self.enc_tcc_k(x[:,9:12,:,:])
        H_, W_ = h_tcc.shape[-2],h_tcc.shape[-1]
        z_x = torch.stack((h_u10, h_v10, h_t2m, h_tcc), dim=1) # b*t,4,c,h,w
        z_x = z_x.reshape(B,T,4,-1,H_, W_).permute(0,2,1,3,4,5).reshape(B,4,-1,H_, W_)
        self.hid_k.train()
        ...
        h_y_u10, _ = self.enc_u10_k(y[:,0:3,:,:])
        h_y_v10, _ = self.enc_v10_k(y[:,3:6,:,:])
        h_y_t2m, _ = self.enc_t2m_k(y[:,6:9,:,:])
        h_y_tcc, _ = self.enc_tcc_k(y[:,9:12,:,:])
        z_y = torch.stack((h_y_u10, h_y_v10, h_y_t2m, h_y_tcc), dim=1) # b*t,4,c,h,w
        ...
        # 3D DEC
        pre_u10 = self.dec_u10_k(h_pre_u10.reshape(B,T,-1,H_,W_).permute(0,2,1,3,4))
        pre_v10 = self.dec_v10_k(h_pre_v10.reshape(B,T,-1,H_,W_).permute(0,2,1,3,4))
        pre_t2m = self.dec_t2m_k(h_pre_t2m.reshape(B,T,-1,H_,W_).permute(0,2,1,3,4))
        pre_tcc = self.dec_tcc_k(h_pre_tcc.reshape(B,T,-1,H_,W_).permute(0,2,1,3,4))
        pre_y = torch.cat((pre_u10, pre_v10, pre_t2m, pre_tcc), dim=1)
...
class Decoder(nn.Module):
    ...
    def __init__(self, C_hid, C_out, N_S, spatio_kernel, act_inplace=True):
        ...
        self.dec = nn.Sequential(
            *[ConvSC3D(C_hid, C_hid, spatio_kernel, upsampling=s,
                     act_inplace=act_inplace) for s in samplings[:-1]],
              ConvSC3D(C_hid, C_hid, spatio_kernel, upsampling=samplings[-1],
                     act_inplace=act_inplace)
        )
...
if __name__ == '__main__':
    ...
    data = torch.randn((16, 4, 12, 32, 64)).to(device)
    model = SimVP_Model(in_shape=[4,12,32,64],hid_S=32,N_S=2,N_T=8,hid_T=256).to(device)
