...
from openstl.models.simvp_ema.video_gpt_modules.videogpt_modules import Encoder_mv, Decoder_mv
...
class SimVP_Model(nn.Module):
    ...
    def __init__(self, in_shape, hid_S=16, hid_T=256, N_S=4, N_T=4, model_type='gSTA',
                 ...
                 spatio_kernel_dec=3, act_inplace=True, momentum_ema=0.9,embedding_dim=8,downsample=(2,2,2),n_res_layers=4,**kwargs):
        ...
        act_inplace = False
        T1 = int(T/downsample[0])
        self.emb_dim = embedding_dim
        self.enc_u10_q = Encoder_mv(n_hiddens=hid_S,embedding_dim=embedding_dim,n_res_layers=n_res_layers,downsample=downsample,in_channels=1)
        self.enc_u10_k = Encoder_mv(n_hiddens=hid_S,embedding_dim=embedding_dim,n_res_layers=n_res_layers,downsample=downsample,in_channels=1)

        self.enc_v10_q = Encoder_mv(n_hiddens=hid_S,embedding_dim=embedding_dim,n_res_layers=n_res_layers,downsample=downsample,in_channels=1)
        self.enc_v10_k = Encoder_mv(n_hiddens=hid_S,embedding_dim=embedding_dim,n_res_layers=n_res_layers,downsample=downsample,in_channels=1)

        self.enc_t2m_q = Encoder_mv(n_hiddens=hid_S,embedding_dim=embedding_dim,n_res_layers=n_res_layers,downsample=downsample,in_channels=1)
        self.enc_t2m_k = Encoder_mv(n_hiddens=hid_S,embedding_dim=embedding_dim,n_res_layers=n_res_layers,downsample=downsample,in_channels=1)

        self.enc_tcc_q = Encoder_mv(n_hiddens=hid_S,embedding_dim=embedding_dim,n_res_layers=n_res_layers,downsample=downsample,in_channels=1)
        self.enc_tcc_k = Encoder_mv(n_hiddens=hid_S,embedding_dim=embedding_dim,n_res_layers=n_res_layers,downsample=downsample,in_channels=1)

        self.dec_u10_q = Decoder_mv(n_hiddens=hid_S,embedding_dim=embedding_dim,n_res_layers=n_res_layers,downsample=downsample,out_channels=1)
        self.dec_u10_k = Decoder_mv(n_hiddens=hid_S,embedding_dim=embedding_dim,n_res_layers=n_res_layers,downsample=downsample,out_channels=1)

        self.dec_v10_q = Decoder_mv(n_hiddens=hid_S,embedding_dim=embedding_dim,n_res_layers=n_res_layers,downsample=downsample,out_channels=1)
        self.dec_v10_k = Decoder_mv(n_hiddens=hid_S,embedding_dim=embedding_dim,n_res_layers=n_res_layers,downsample=downsample,out_channels=1)

        self.dec_t2m_q = Decoder_mv(n_hiddens=hid_S,embedding_dim=embedding_dim,n_res_layers=n_res_layers,downsample=downsample,out_channels=1)
        self.dec_t2m_k = Decoder_mv(n_hiddens=hid_S,embedding_dim=embedding_dim,n_res_layers=n_res_layers,downsample=downsample,out_channels=1)

        self.dec_tcc_q = Decoder_mv(n_hiddens=hid_S,embedding_dim=embedding_dim,n_res_layers=n_res_layers,downsample=downsample,out_channels=1)
        self.dec_tcc_k = Decoder_mv(n_hiddens=hid_S,embedding_dim=embedding_dim,n_res_layers=n_res_layers,downsample=downsample,out_channels=1)

        self.init_weights()

        model_type = 'gsta' if model_type is None else model_type.lower()
        if model_type == 'incepu':
            self.hid = MidIncepNet(T1*embedding_dim*C, hid_T, N_T)
        else:
            self.hid = MidMetaNet(T1*embedding_dim*C, hid_T, N_T,
    ...
    def forward(self, x_raw, y_raw, **kwargs):
        ...
        # mask_x,_ = self.mask_replace_blocks(x_raw.clone())
        _, T1, _, _, _ = y_raw.shape
        x = x_raw.clone()
        y = y_raw.clone()
        # 重建
        h_u10 = self.enc_u10_q(x[:,:,0:1,...])
        h_v10 = self.enc_v10_q(x[:,:,1:2,...])
        h_t2m = self.enc_t2m_q(x[:,:,2:3,...])
        h_tcc = self.enc_tcc_q(x[:,:,3:4,...])

        rec_u10 = self.dec_u10_q(h_u10)
        rec_v10 = self.dec_v10_q(h_v10)
        rec_t2m = self.dec_t2m_q(h_t2m)
        rec_tcc = self.dec_tcc_q(h_tcc)
        rec_x = torch.cat((rec_u10, rec_v10, rec_t2m, rec_tcc), dim=2)
        ...
        h_u10 = self.enc_u10_k(x[:,:,0:1,...])
        h_v10 = self.enc_v10_k(x[:,:,1:2,...])
        h_t2m = self.enc_t2m_k(x[:,:,2:3,...])
        h_tcc = self.enc_tcc_k(x[:,:,3:4,...])
        H_, W_ = h_tcc.shape[-2],h_tcc.shape[-1]
        z = torch.cat((h_u10, h_v10, h_t2m, h_tcc), dim=1) # b c t h w
        z = z.permute(0,2,1,3,4)
        hid = self.hid(z)

        hid = hid.permute(0,2,1,3,4)
        h_pre_u10 = hid[:,:self.emb_dim,...]
        h_pre_v10 = hid[:,self.emb_dim:2*self.emb_dim,...]
        h_pre_t2m = hid[:,2*self.emb_dim:3*self.emb_dim,...]
        h_pre_tcc = hid[:,3*self.emb_dim:4*self.emb_dim,...]

        h_y_u10 = self.enc_u10_k(y[:,:,0:1,...])
        h_y_v10 = self.enc_v10_k(y[:,:,1:2,...])
        h_y_t2m = self.enc_t2m_k(y[:,:,2:3,...])
        h_y_tcc = self.enc_tcc_k(y[:,:,3:4,...])
        ...
        pre_y = torch.cat((pre_u10, pre_v10, pre_t2m, pre_tcc), dim=2)
...
if __name__ == '__main__':
    ...
    data = torch.randn((16, 12, 4, 32, 64)).to(device)
