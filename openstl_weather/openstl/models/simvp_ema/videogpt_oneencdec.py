...
class SimVP_Model(nn.Module):
    ...
    def __init__(self, in_shape, hid_S=16, hid_T=256, N_S=4, N_T=4, model_type='gSTA',
        ...
        self.enc_q = Encoder_mv(n_hiddens=hid_S,embedding_dim=embedding_dim,n_res_layers=n_res_layers,downsample=downsample,in_channels=1)
        self.enc_k = Encoder_mv(n_hiddens=hid_S,embedding_dim=embedding_dim,n_res_layers=n_res_layers,downsample=downsample,in_channels=1)


        self.dec_q = Decoder_mv(n_hiddens=hid_S,embedding_dim=embedding_dim,n_res_layers=n_res_layers,downsample=downsample,out_channels=1)
        self.dec_k = Decoder_mv(n_hiddens=hid_S,embedding_dim=embedding_dim,n_res_layers=n_res_layers,downsample=downsample,out_channels=1)
    ...
    def init_weights(self):
        ...
        """
        for param_q, param_k in zip(self.enc_q.parameters(),
                                    self.enc_k.parameters()):
            param_k.requires_grad = False
            param_k.data.copy_(param_q.data)
        
        for param_q, param_k in zip(self.dec_q.parameters(),
                                    self.dec_k.parameters()):
            param_k.requires_grad = False
            param_k.data.copy_(param_q.data)
    ...
    def forward(self, x_raw, y_raw, **kwargs):
        ...
        x = x_raw.clone().permute(0,2,1,3,4)
        x = x.reshape(B*C,T,1,H,W)
        y = y_raw.clone().permute(0,2,1,3,4)
        y = y.reshape(B*C,T1,1,H,W)
        # 重建
        h = self.enc_q(x)
        rec_x = self.dec_q(h).reshape(B,C,T,H,W).permute(0,2,1,3,4)
        ...
        T_, H_, W_ = z.shape[-3],z.shape[-2],z.shape[-1]
   
        z = z.reshape(B,C,-1,H_,W_)
        hid = self.hid(z)
        hid = hid.reshape(B*C,-1,T_,H_,W_)
        h_y = self.enc_k(y)
        loss_latent = F.mse_loss(hid,h_y)

        pre_y = self.dec_k(hid).reshape(B,C,T,H,W).permute(0,2,1,3,4)

        loss_pre = F.mse_loss(pre_y,y_raw)

        loss = loss_rec + 0.01*loss_latent + 0.001*loss_pre
