...
class SimVP_Model(nn.Module):
    ...
    def __init__(self, in_shape, hid_S=16, hid_T=256, N_S=4, N_T=4, model_type='gSTA',
        ...
        if model_type == 'incepu':
            self.hid_q = MidIncepNet(T*hid_S, hid_T, N_T)
            self.hid_k = MidIncepNet(T*hid_S, hid_T, N_T)
        else:
            self.hid_q = MidMetaNet(T*hid_S, hid_T, N_T,
                input_resolution=(H, W), model_type=model_type,
                mlp_ratio=mlp_ratio, drop=drop, drop_path=drop_path)
            self.hid_k = MidMetaNet(T*hid_S, hid_T, N_T,
                input_resolution=(H, W), model_type=model_type,
                mlp_ratio=mlp_ratio, drop=drop, drop_path=drop_path)
    ...
    def forward(self, x_raw, y_raw, **kwargs):
        ...
        z_x = z_x.reshape(B,T,4,-1,H_,W_).permute(0,2,1,3,4,5).reshape(B*4,T,-1,H_,W_)
        self.hid_k.train()
        z_y_pre = self.hid_k(z_x).reshape(B,4,T,-1,H_,W_).permute(0,2,1,3,4,5).reshape(B*T,4,-1,H_,W_)
        h_pre_u10 = z_y_pre[:,0,:,:]
        h_pre_v10 = z_y_pre[:,1,:,:]
        h_pre_t2m = z_y_pre[:,2,:,:]
        h_pre_tcc = z_y_pre[:,3,:,:]

        y_blur = self.gs_blur(y)
        h_y_u10, _ = self.enc_u10_k(y_blur[:,0:1,:,:])
        h_y_v10, _ = self.enc_v10_k(y_blur[:,1:2,:,:])
        h_y_t2m, _ = self.enc_t2m_k(y_blur[:,2:3,:,:])
        h_y_tcc, _ = self.enc_tcc_k(y_blur[:,3:4,:,:])
        z_y = torch.cat((h_y_u10, h_y_v10, h_y_t2m, h_y_tcc), dim=1).reshape(B*T,-1,H_,W_)
        ...
        #     lmd = 0
        # # mix
        # lmd = np.random.beta(2, 2)
        # only x
        lmd = 1
        h_rec_u10 = (1-lmd) * h_y_u10 + lmd * h_pre_u10
        h_rec_v10 = (1-lmd) * h_y_v10 + lmd * h_pre_v10
        h_rec_t2m = (1-lmd) * h_y_t2m + lmd * h_pre_t2m
        h_rec_tcc = (1-lmd) * h_y_tcc + lmd * h_pre_tcc
        ...
        h_u10, _ = self.enc_u10_k(x[:,0:1,:,:])
        h_v10, _ = self.enc_v10_k(x[:,1:2,:,:])
        h_t2m, _ = self.enc_t2m_k(x[:,2:3,:,:])
        h_tcc, _ = self.enc_tcc_k(x[:,3:4,:,:])
        H_, W_ = h_tcc.shape[-2],h_tcc.shape[-1]
        # z_x = torch.cat((h_u10, h_v10, h_t2m, h_tcc), dim=1)
        # z_x = z_x.reshape(B,T,-1,H_,W_)
        z_x = torch.stack((h_u10, h_v10, h_t2m, h_tcc), dim=1) # b*t 4 c h w
        z_x = z_x.reshape(B,T,4,-1,H_,W_).permute(0,2,1,3,4,5).reshape(B*4,T,-1,H_,W_)
        z_y_pre = self.hid_q(z_x).reshape(B,4,T,-1,H_,W_).permute(0,2,1,3,4,5).reshape(B*T,4,-1,H_,W_)
        ...
        z_y = torch.stack((h_y_u10, h_y_v10, h_y_t2m, h_y_tcc), dim=1)
