...
class SimVP_Model(nn.Module):
    ...
    def __init__(self, in_shape, hid_S=16, hid_T=256, N_S=4, N_T=4, model_type='gSTA',
        ...
        self.enc_q = Encoder(C, hid_S, N_S, spatio_kernel_enc, act_inplace=act_inplace)
        self.enc_k = Encoder(C, hid_S, N_S, spatio_kernel_enc, act_inplace=act_inplace)

        self.dec_q = Decoder(hid_S, C, N_S, spatio_kernel_dec, act_inplace=act_inplace)
        self.dec_k = Decoder(hid_S, C, N_S, spatio_kernel_dec, act_inplace=act_inplace)
    ...
    def forward(self, x_raw, y_raw, **kwargs):
        ...
        # 一阶段
        z_x, _ = self.enc_q(x)
        H_, W_ = z_x.shape[-2],z_x.shape[-1]
        z_x = z_x.reshape(B,1,-1,H_,W_)

        self.hid_k.train()
        z_y_pre = self.hid_k(z_x).reshape(B*T,-1, H_, W_)

        rec_y = self.dec_q(z_y_pre)
...
if __name__ == '__main__':
    ...
    data = torch.randn((16, 12, 1, 32, 64)).to(device)
    model = SimVP_Model(in_shape=[12,1,32,64],hid_S=32,N_S=2,N_T=8,hid_T=256).to(device)
