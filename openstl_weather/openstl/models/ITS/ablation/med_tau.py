...
class SimVP_Model(nn.Module):
    ...
    def forward(self, x_raw, y_raw, **kwargs):
        ...
        # 重建
        z1, _ = self.enc_1(x[:,:2,...])
        z2, _ = self.enc_2(x[:,2:3,...]) 
        z3, _ = self.enc_3(x[:,3:4,...])
        z4, _ = self.enc_4(x[:,4:,...]) 
        z = torch.stack((z1, z2, z3, z4), dim=1) # b*t,4,c,h,w

        H_, W_ = z.shape[-2],z.shape[-1]
        z = z.reshape(B,T,-1, H_, W_)
        hid = self.hid(z)
        hid = hid.reshape(B*T, 4,-1, H_, W_)

        pre_y_1 = self.dec_1(hid[:,0,...])
        pre_y_2 = self.dec_2(hid[:,1,...])
        pre_y_3 = self.dec_3(hid[:,2,...])
        pre_y_4 = self.dec_4(hid[:,3,...])

        pre_y = torch.cat((pre_y_1, pre_y_2, pre_y_3, pre_y_4), dim=1)
