...
from openstl.models.ITS.module.path_ciatt import PredFormer
...
class SimVP_Model(nn.Module):
    ...
    def __init__(self, in_shape, hid_S=16, hid_T=256, N_S=4, N_T=4, model_type='gSTA',
        ...
        self.dec_u10_q = Decoder(hid_S, 2*T, N_S, spatio_kernel_dec, act_inplace=act_inplace,T=T)
        self.dec_u10_k = Decoder(hid_S, 2*T, N_S, spatio_kernel_dec, act_inplace=act_inplace,T=T)

        self.dec_v10_q = Decoder(hid_S, 1*T, N_S, spatio_kernel_dec, act_inplace=act_inplace,T=T)
        self.dec_v10_k = Decoder(hid_S, 1*T, N_S, spatio_kernel_dec, act_inplace=act_inplace,T=T)

        self.dec_t2m_q = Decoder(hid_S, 1*T, N_S, spatio_kernel_dec, act_inplace=act_inplace,T=T)
        self.dec_t2m_k = Decoder(hid_S, 1*T, N_S, spatio_kernel_dec, act_inplace=act_inplace,T=T)

        self.dec_tcc_q = Decoder(hid_S, 1*T, N_S, spatio_kernel_dec, act_inplace=act_inplace,T=T)
        self.dec_tcc_k = Decoder(hid_S, 1*T, N_S, spatio_kernel_dec, act_inplace=act_inplace,T=T)
        ...
        self.hid_q = PredFormer(dim = hid_T,channels=hid_S,depth=N_T)
        self.hid_k = PredFormer(dim = hid_T,channels=hid_S,depth=N_T)
    ...
    def forward(self, x_raw, y_raw, **kwargs):
        ...
        z_x = z_x.reshape(B,T,4,-1,H_, W_).permute(0,2,1,3,4,5)
        self.hid_k.train()
        ...
        # 3D DEC
        rec_u10 = self.dec_u10_q(h_pre_u10.reshape(B,-1,H_, W_)).reshape(B,T,-1,H,W)
        rec_v10 = self.dec_v10_q(h_pre_v10.reshape(B,-1,H_, W_)).reshape(B,T,-1,H,W)
        rec_t2m = self.dec_t2m_q(h_pre_t2m.reshape(B,-1,H_, W_)).reshape(B,T,-1,H,W)
        rec_tcc = self.dec_tcc_q(h_pre_tcc.reshape(B,-1,H_, W_)).reshape(B,T,-1,H,W)
        rec_y = torch.cat((rec_u10, rec_v10, rec_t2m, rec_tcc), dim=2)
        ...
        z_y = torch.stack((h_y_u10, h_y_v10, h_y_t2m, h_y_tcc), dim=1).reshape(B,T,4,-1,H_,W_).permute(0,2,1,3,4,5) # b*t,4,c,h,w
        ...
        # 3D DEC
        pre_u10 = self.dec_u10_k(h_pre_u10.reshape(B,-1,H_, W_)).reshape(B,T,-1,H,W)
        pre_v10 = self.dec_v10_k(h_pre_v10.reshape(B,-1,H_, W_)).reshape(B,T,-1,H,W)
        pre_t2m = self.dec_t2m_k(h_pre_t2m.reshape(B,-1,H_, W_)).reshape(B,T,-1,H,W)
        pre_tcc = self.dec_tcc_k(h_pre_tcc.reshape(B,-1,H_, W_)).reshape(B,T,-1,H,W)
...
class Decoder(nn.Module):
    ...
    def __init__(self, C_hid, C_out, N_S, spatio_kernel, act_inplace=True,T = 12):
        samplings = sampling_generator(N_S, reverse=True)
        super(Decoder, self).__init__()
        self.conv1 = ConvSC(T*C_hid,C_hid,spatio_kernel)
