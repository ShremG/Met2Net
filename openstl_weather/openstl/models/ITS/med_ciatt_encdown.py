...
class SimVP_Model(nn.Module):
    ...
    def __init__(self, in_shape, hid_S=16, hid_T=256, N_S=4, N_T=4, model_type='gSTA',
        ...
        self.enc_u10_q = EncoderDown(2, hid_S, N_S, spatio_kernel_enc, act_inplace=act_inplace)
        self.enc_u10_k = EncoderDown(2, hid_S, N_S, spatio_kernel_enc, act_inplace=act_inplace)

        self.enc_v10_q = EncoderDown(1, hid_S, N_S, spatio_kernel_enc, act_inplace=act_inplace)
        self.enc_v10_k = EncoderDown(1, hid_S, N_S, spatio_kernel_enc, act_inplace=act_inplace)

        self.enc_t2m_q = EncoderDown(1, hid_S, N_S, spatio_kernel_enc, act_inplace=act_inplace)
        self.enc_t2m_k = EncoderDown(1, hid_S, N_S, spatio_kernel_enc, act_inplace=act_inplace)

        self.enc_tcc_q = EncoderDown(1, hid_S, N_S, spatio_kernel_enc, act_inplace=act_inplace)
        self.enc_tcc_k = EncoderDown(1, hid_S, N_S, spatio_kernel_enc, act_inplace=act_inplace)

        self.dec_u10_q = DecoderUp(hid_S, 2, N_S, spatio_kernel_dec, act_inplace=act_inplace)
        self.dec_u10_k = DecoderUp(hid_S, 2, N_S, spatio_kernel_dec, act_inplace=act_inplace)

        self.dec_v10_q = DecoderUp(hid_S, 1, N_S, spatio_kernel_dec, act_inplace=act_inplace)
        self.dec_v10_k = DecoderUp(hid_S, 1, N_S, spatio_kernel_dec, act_inplace=act_inplace)

        self.dec_t2m_q = DecoderUp(hid_S, 1, N_S, spatio_kernel_dec, act_inplace=act_inplace)
        self.dec_t2m_k = DecoderUp(hid_S, 1, N_S, spatio_kernel_dec, act_inplace=act_inplace)

        self.dec_tcc_q = DecoderUp(hid_S, 1, N_S, spatio_kernel_dec, act_inplace=act_inplace)
        self.dec_tcc_k = DecoderUp(hid_S, 1, N_S, spatio_kernel_dec, act_inplace=act_inplace)
        ...
        self.hid_q = CIMidNet(in_channels=T*hid_S*4,d_model=hid_T, n_layers=N_T, heads=8)
        self.hid_k = CIMidNet(in_channels=T*hid_S*4,d_model=hid_T, n_layers=N_T, heads=8)
        self.init_weights()
...
class EncoderDown(nn.Module):
    ...
    def __init__(self, C_in, C_hid, N_S, spatio_kernel, act_inplace=True):
        super(EncoderDown, self).__init__()
        # samplings = sampling_generator(N_S)
        self.emb =  ConvSC(C_in, C_hid, spatio_kernel, downsampling=False, act_inplace=act_inplace)
        self.enc1 = ConvSC(C_hid, 2*C_hid, spatio_kernel, downsampling=True, act_inplace=act_inplace)
        self.enc2 = ConvSC(2*C_hid, 4*C_hid, spatio_kernel, downsampling=True, act_inplace=act_inplace)
        ...
        # )
        self.res_cnt1 = PixelUnshuffleChannelAveragingDownSampleLayer(C_hid, 2*C_hid, 2)
        self.res_cnt2 = PixelUnshuffleChannelAveragingDownSampleLayer(2*C_hid, 4*C_hid, 2)

    def forward(self, x):  # B*4, 3, 128, 128
        emb = self.emb(x)
        enc1 = emb.clone()

        emb = self.enc1(emb) + self.res_cnt1(emb)
        emb = self.enc2(emb) + self.res_cnt2(emb)
...
class DecoderUp(nn.Module):
    ...
    def __init__(self, C_hid, C_out, N_S, spatio_kernel, act_inplace=True):
        super(DecoderUp, self).__init__()
        # samplings = sampling_generator(N_S)
        self.dec1 =  ConvSC(4*C_hid, 2*C_hid, spatio_kernel, upsampling=True,act_inplace=act_inplace)
        self.dec2 = ConvSC(2*C_hid, C_hid, spatio_kernel, upsampling=True,act_inplace=act_inplace)
        self.readout = nn.Conv2d(C_hid, C_out, 1)
        self.res_cnt1 = ChannelDuplicatingPixelUnshuffleUpSampleLayer(4*C_hid, 2*C_hid, 2)
        self.res_cnt2 = ChannelDuplicatingPixelUnshuffleUpSampleLayer(2*C_hid, C_hid, 2)
        

    def forward(self, x):  # B*4, 3, 128, 128

        y = self.dec1(x) + self.res_cnt1(x)
        y = self.dec2(y) + self.res_cnt2(y)
        y = self.readout(y)
