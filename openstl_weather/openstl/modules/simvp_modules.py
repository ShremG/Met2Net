...
class StokenAttention(nn.Module):
    def __init__(self, dim, stoken_size=[2,2], n_iter=1, kernel_size=21):
        super().__init__()
        ...
        self.stoken_refine = TemporalAttention(dim, kernel_size)
...
class TAUSubBlock_superToken(GASubBlock):
    """A TAUBlock (tau) for Temporal Attention Unit"""

    def __init__(self, dim, kernel_size=21, mlp_ratio=4.,stoken_size = [2,2],
                 drop=0., drop_path=0.1, init_value=1e-2, act_layer=nn.GELU):
        ...
        self.attn = StokenAttention(dim=dim,stoken_size=stoken_size,kernel_size = kernel_size)
