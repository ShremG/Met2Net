...
class BaseExperiment(object):
    ...
    def __init__(self, args, dataloaders=None, strategy='auto'):
        ...
        self.config['data_name'] = self.data.data_name

        # import numpy as np
        # np.save('/storage/linhaitao/lsh/openstl_weather/tools/vis/mean.npy',self.data.test_mean)
        # np.save('/storage/linhaitao/lsh/openstl_weather/tools/vis/std.npy',self.data.test_std)
        # raise ValueError('===============================================')
    ...
    def test(self):
        if self.args.test == True:
            ...
            print('LOAD MODEL:',osp.join(self.save_dir, 'checkpoints', 'best.ckpt'))
    ...
    def display_method_info(self, args):
        ...
        elif args.method in ['simvp_ema','simvp_ema_ae','its']:
            input_dummy = (torch.ones(1, args.pre_seq_length, C, H, W).to(device),torch.ones(1, args.pre_seq_length, C, H, W).to(device))
            # input_dummy = torch.ones(1, args.pre_seq_length, C, H, W).to(device)
