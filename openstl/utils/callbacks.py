...
class EpochEndCallback(Callback):
    ...
    def on_validation_epoch_end(self, trainer, pl_module):
        ...
        rec_val_loss = trainer.callback_metrics.get('rec_loss')
        latent_val_loss = trainer.callback_metrics.get('latent_loss')
        pre_val_loss = trainer.callback_metrics.get('pre_loss')

        if hasattr(self, 'avg_train_loss'):
            if latent_val_loss != None:
                print_log(f"Epoch {trainer.current_epoch}: Lr: {lr:.7f} | Train Loss: {self.avg_train_loss:.7f} | Vali Loss: {avg_val_loss:.7f} | Rec Loss: {rec_val_loss:.7f} | Latent Loss: {latent_val_loss:.7f} | Pre Loss: {pre_val_loss:.7f}")
            else:
